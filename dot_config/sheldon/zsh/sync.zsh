...
export PATH=$HOME/.local/share/chezmoi/nvim-macos-arm64/bin:$PATH
export GOBIN=$HOME/workspace/go/go/bin
export GOROOT_BOOTSTRAP=$HOME/workspace/go/go-darwin-arm64-bootstrap
export GPG_TTY=$(tty)
export PATH=$HOME/workspace/zig/bin:$PATH
export PATH=$HOME/workspace/go/tinygo/build:$PATH
export TINYGO_GOCOMPATIBILITY=0
export HOMEBREW_FORBIDDEN_FORMULAE="node python python3 pip npm pnpm yarn claude go"
...
function gr() {
    ...
    local repo="$(cat ~/.local/share/chezmoi/ghq_list.txt | fzf --preview="ls -AF --color=always ${root}/{1}")"
    local dir="${root}/${repo}"
    [ -n "${dir}" ] && cd "${dir}"
}
